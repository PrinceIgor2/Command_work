from functions import read_from_txt, read_from_csv, write_list_to_csv
import os.path
from os import path
...
def import_data(path_file_phone_db, coding):
    ...
    if os.path.exists(path_file_import) == True:        # проверка на наличие файла
        ...
        if extension == '.txt':
            data_txt = read_from_txt(path_file_import, coding)
            with open(path_file_phone_db, 'a+', encoding=coding) as ph_db:
                ph_db.write(f'\n')
                for i in data_txt:
                    ph_db.write(f'{i}')
        elif extension == '.csv':
            data_csv = read_from_csv(path_file_import, coding)
            write_list_to_csv(path_file_phone_db, coding, data_csv)
        else:
            print(f'Импорт контактов из файла с расширением {extension} не возможен.')
